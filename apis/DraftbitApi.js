...
export const productsGET = async (Constants, _args, handlers, timeout) => {
  const paramsDict = {};
  paramsDict['_limit'] = '1';
...
export const useTeamsGET = (
  ...
  const queryClient = useQueryClient();
  return useQuery(
    ['draftbitTeamsGET', args],
    () => teamsGET(Constants, args, handlers, timeout),
    {
      refetchInterval,
      refetchOnWindowFocus,
      refetchOnMount,
      ...
      retry,
      staleTime,
      onSuccess: () => queryClient.invalidateQueries(['draftbitTeamsGETS']),
    }
  );
};
