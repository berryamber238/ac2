...
const OrganizerArticleBlock = props => {
  const { theme } = props;
  ...
  return (
    <View>
      ...
      <AceCampTestApi.FetchOrganizerMinute$article$eventGET
        handlers={{
          onData: articleGetData => {
            const handler = async () => {
              try {
                setIsLoading(true);
                setEventData(eventData.concat(articleGetData?.data?.feeds));
                setCursor(
                  (() => {
                    const e = articleGetData?.data?.feeds;
                    return e[e.length - 1];
                  })()?.cursor
                );
                setTotalPages(articleGetData?.meta?.total_pages);
      ...
      >
        {({ loading, error, data, refetchOrganizerMinute$article$event }) => {
          const articleGetData = data?.json;
          if (loading) {
              ...
              >
                ...
                >
                  <SimpleStyleFlashList
                    ...
                    listKey={
                      'article get->View->Shadow->系列活动-列表->FlashList'
                    }
                    numColumns={1}
                    onEndReached={() => {
                      const handler = async () => {
                        try {
                          ...
                          if (
                            eventData?.length >= articleGetData?.meta?.total
                          ) {
                            return;
                          }
                          setIsLoading(true);
                    ...
                    initialNumToRender={articleGetData?.meta?.total}
...
export default withTheme(OrganizerArticleBlock);
