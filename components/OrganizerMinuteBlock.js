...
const OrganizerMinuteBlock = props => {
  const { theme } = props;
  ...
  return (
    <View>
      ...
      <AceCampTestApi.FetchOrganizerMinute$article$eventGET
        handlers={{
          onData: minuteGetData => {
            const handler = async () => {
              try {
                setIsLoading(true);
                setEventData(eventData.concat(minuteGetData?.data?.feeds));
                setCursor(
                  (() => {
                    const e = minuteGetData?.data?.feeds;
                    return e[e.length - 1];
                  })()?.cursor
                );
                setTotalPages(minuteGetData?.meta?.total_pages);
...
export default withTheme(OrganizerMinuteBlock);
